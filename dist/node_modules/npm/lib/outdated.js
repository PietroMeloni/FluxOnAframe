...
function outdated_(args, path, tree, parentHas, depth, cb) {
  ...
  var deps = tree.error ? tree.children : tree.children.filter(function (child) {
...
function shouldUpdate(args, tree, dep, has, req, depth, pkgpath, cb, type) {
  ...
  function updateLocalDeps(latestRegistryVersion) {
    fetchPackageMetadata('file:' + parsed.fetchSpec, '.', function (er, localDependency) {
      if (er) return cb();
  ...
  function updateDeps(er, d) {
    ...
    if (!curr || dFromUrl && cFromUrl && m._from !== curr.from || m.version !== curr.version || m.version !== l.version) {
