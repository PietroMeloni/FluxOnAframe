    ...
    Object.keys(d).forEach(function (k) {
      ...
      if (Array.isArray(d[k])) {
        d[k].forEach(function (val, i) {
          var pi = p + '[' + i + ']';
          if (val && (typeof val === 'undefined' ? 'undefined' : _typeof(val)) === 'object') getFields(val, f, [p]);else f.push(pi);
        });
        return;
      }
      if (_typeof(d[k]) === 'object') getFields(d[k], f, [p]);
    });
    return f;
  }
};
...
function fetchAndRead(nv, args, silent, cb) {
  ...
  mapToRegistry(name, npm.config, function (er, uri, auth) {
    ...
    npm.registry.get(uri, { auth: auth }, function (er, data) {
      ...
      if (error || silent) {
        cb(error, retval);
      } else if (!npm.config.get('json') && args.length === 1 && args[0] === '') {
        data.version = version;
        BB.all(results.map(function (v) {
          return prettyView(data, v[Object.keys(v)[0]]['']);
        })).nodeify(cb).then(function () {
          return retval;
        });
      } else {
...
function prettyView(packument, manifest) {
  ...
  var unicode = npm.config.get('unicode');
  return BB.try(function () {
    if (!manifest) {
      log.error('view', 'No matching versions.\n' + 'To see a list of versions, run:\n' + ('> npm view ' + packument.name + ' versions'));
      return;
    }
    var tags = [];
    Object.keys(packument['dist-tags']).forEach(function (t) {
      var version = packument['dist-tags'][t];
      tags.push(style.bright(color.green(t)) + ': ' + version);
    });
    var unpackedSize = manifest.dist.unpackedSize && byteSize(manifest.dist.unpackedSize);
    var licenseField = manifest.license || manifest.licence || 'Proprietary';
    var info = {
      ...
      deps: Object.keys(manifest.dependencies || {}).map(function (dep) {
      ...
      maintainers: (packument.maintainers || []).map(function (u) {
        return unparsePerson({
    ...
    if (info.maintainers && info.maintainers.length) {
      ...
      info.maintainers.forEach(function (u) {
...
function printData(data, name, cb) {
  ...
  process.stdout.write(msg, function () {
    return cb(null, data);
  });
}
function cleanup(data) {
  ...
  if (_typeof(data.versions) === 'object' && data.versions && !Array.isArray(data.versions)) {
    data.versions = Object.keys(data.versions || {});
  }

  var keys = Object.keys(data);
  keys.forEach(function (d) {
    if (d.charAt(0) === '_') delete data[d];else if (_typeof(data[d]) === 'object') data[d] = cleanup(data[d]);
  });
