...
function readLockfile(name) {
  return readFile(path.join(npm.localPrefix, name), 'utf8').catch({ code: 'ENOENT' }, function () {
    return null;
  });
}

function updateShrinkwrap(newVersion, cb) {
  BB.join(readLockfile(SHRINKWRAP), readLockfile(PKGLOCK), function (shrinkwrap, lockfile) {
    if (!shrinkwrap && !lockfile) {
      return cb(null, false, false);
    }
    var file = shrinkwrap ? SHRINKWRAP : PKGLOCK;
    var data = void 0;
    var indent = void 0;
    var newline = void 0;
    try {
    ...
    data.version = newVersion;
    write(data, file, indent, newline, function (err) {
      if (err) {
...
function _commit(version, localData, cb) {
  ...
  var commitArgs = buildCommitArgs(['commit', signCommit ? '-S -m' : '-m', message]);
  var flagForTag = signTag ? '-sm' : '-am';

  stagePackageFiles(localData, options).then(function () {
    return git.exec(commitArgs, options);
  }).then(function () {
    if (!localData.existingTag) {
      return git.exec(['tag', npm.config.get('tag-version-prefix') + version, flagForTag, message], options);
    }
  }).nodeify(cb);
}

function stagePackageFiles(localData, options) {
  return addLocalFile('package.json', options, false).then(function () {
...
function addLocalFile(file, options, ignoreFailure) {
  var p = git.exec(['add', path.join(npm.localPrefix, file)], options);
  return ignoreFailure ? p.catch(function () {}) : p;
}

function write(data, file, indent, newline, cb) {
  assert(data && (typeof data === 'undefined' ? 'undefined' : _typeof(data)) === 'object', 'must pass data to version write');
  assert(typeof file === 'string', 'must pass filename to write to version write');

  log.verbose('version.write', 'data', data, 'to', file);
  writeFileAtomic(path.join(npm.localPrefix, file), stringifyPackage(data, indent, newline), cb);
}
