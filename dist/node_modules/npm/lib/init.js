  ...
  if (args.length) {
    ...
    if (/^@[^/]+$/.test(initerName)) {
      ...
      if (req.type === 'git' && req.hosted) {
        var _req$hosted = req.hosted,
            user = _req$hosted.user,
            project = _req$hosted.project;
    ...
    var npxArgs = [process.argv0, '[fake arg]', '--always-spawn', packageName].concat(_toConsumableArray(process.argv.slice(4)));
    var parsed = npx.parseArgs(npxArgs, NPM_PATH);

    return npx(parsed).then(function () {
      return cb();
    }).catch(cb);
  }
  var dir = process.cwd();
