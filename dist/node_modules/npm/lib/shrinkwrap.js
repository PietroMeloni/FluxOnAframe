...
var readFile = BB.promisify(require('graceful-fs').readFile);
var getRequested = require('./install/get-requested.js');
var id = require('./install/deps.js');
var iferr = require('iferr');
var isOnlyOptional = require('./install/is-only-optional.js');
...
var readPackageTree = BB.promisify(require('read-package-tree'));
var ssri = require('ssri');
var stringifyPackage = require('./utils/stringify-package');
var validate = require('aproba');
var writeFileAtomic = require('write-file-atomic');
var unixFormatPath = require('./utils/unix-format-path.js');
var isRegistry = require('./utils/is-registry.js');

var PKGLOCK = 'package-lock.json';
var SHRINKWRAP = 'npm-shrinkwrap.json';
var PKGLOCK_VERSION = npm.lockfileVersion;
...
function shrinkwrap(args, silent, cb) {
  ...
  move(path.resolve(npm.prefix, PKGLOCK), path.resolve(npm.prefix, SHRINKWRAP), { Promise: BB }).then(function () {
    log.notice('', PKGLOCK + ' has been renamed to ' + SHRINKWRAP + '. ' + SHRINKWRAP + ' will be used for future installations.');
    return readFile(path.resolve(npm.prefix, SHRINKWRAP)).then(function (d) {
      return JSON.parse(d);
    });
  }, function (err) {
    if (err.code !== 'ENOENT') {
      ...
      return readPackageTree(npm.localPrefix).then(id.computeMetadata).then(function (tree) {
...
function shrinkwrapDeps(deps, top, tree, seen) {
  ...
  sortModules(tree.children).forEach(function (child) {
    ...
    if (child.requires.length) {
      pkginfo.requires = {};
      sortModules(child.requires).forEach(function (required) {
...
function sortModules(modules) {
  ...
  return modules.sort(function (a, b) {
    return sortedModuleNames.indexOf(moduleName(a)) - sortedModuleNames.indexOf(moduleName(b));
  });
}
...
function save(dir, pkginfo, opts, cb) {
  ...
  BB.join(checkPackageFile(dir, SHRINKWRAP), checkPackageFile(dir, PKGLOCK), checkPackageFile(dir, 'package.json'), function (shrinkwrap, lockfile, pkg) {
    var info = shrinkwrap || lockfile || {
      path: path.resolve(dir, opts.defaultFile || PKGLOCK),
      data: '{}',
      indent: pkg && pkg.indent,
      newline: pkg && pkg.newline
    };
    var updated = updateLockfileMetadata(pkginfo, pkg && JSON.parse(pkg.raw));
    var swdata = stringifyPackage(updated, info.indent, info.newline);
    if (swdata === info.raw) {
      ...
      writeFileAtomic(info.path, swdata, function (err) {
...
function updateLockfileMetadata(pkginfo, pkgJson) {
  ...
  var newPkg = {};
  var metainfoWritten = false;
  var metainfo = new Set(['lockfileVersion', 'preserveSymlinks']);
  Object.keys(pkginfo).forEach(function (k) {
    if (k === 'dependencies') {
