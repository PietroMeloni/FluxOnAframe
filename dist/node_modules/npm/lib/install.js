...
function Installer(where, dryrun, args, opts) {
  ...
  var only = npm.config.get('only');
  var onlyProd = /^prod(uction)?$/.test(only);
  var onlyDev = /^dev(elopment)?$/.test(only);
  var prod = npm.config.get('production');
...
Installer.prototype.run = function (_cb) {
  ...
  if (this.packageLockOnly) {
    ...
    if (getSaveType()) {
      ...
      [this, function (next) {
        computeMetadata(_this.idealTree);next();
...
Installer.prototype.normalizeCurrentTree = function (cb) {
  ...
  if (this.currentTree.error) {
    ...
    try {
      for (var _iterator = this.currentTree.children[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  function normalizeTree(tree, seen) {
    ...
    tree.children.forEach(function (child) {
      return normalizeTree(child, seen);
    });
  }
};
...
Installer.prototype.pruneIdealTree = function (cb) {
  ...
  if (!this.idealTree.hasRequiresFromLock && this.idealTree.children.some(function (n) {
    return n.fakeChild;
  })) return cb();
  var toPrune = this.idealTree.children.filter(function (child) {
    return isExtraneous(child) && (_this3.autoPrune || child.removing);
  }).map(function (n) {
    return { name: moduleName(n) };
  });
...
Installer.prototype.loadAllDepsIntoIdealTree = function (cb) {
  ...
  if (installNewModules) {
    ...
    var depsToPreload = Object.assign({}, this.idealTree.package.devDependencies, this.idealTree.package.dependencies);
...
Installer.prototype.rollbackFailedOptional = function (staging, actionsToRun, cb) {
  ...
  Bluebird.map(failed, function (pkg) {
    return Bluebird.map(pkg.rollback, function (rollback) {
      return rollback(top, staging, pkg);
    });
  }).asCallback(cb);
};
...
Installer.prototype.runPreinstallTopLevelLifecycles = function (cb) {
  ...
  readPackageJson(path.join(this.where, 'package.json'), log, false, function (err, data) {
    if (err) return cb();
    _this4.currentTree = createNode({
      isTop: true,
      package: data,
      path: _this4.where
    });
    doOneAction('preinstall', _this4.where, _this4.currentTree, log.newGroup('preinstall:.'), cb);
  });
};
...
Installer.prototype.startAudit = function (cb) {
  ...
  this.auditSubmission = Bluebird.try(function () {
    return audit.generateFromInstall(_this5.idealTree, _this5.differences, _this5.args, _this5.remove);
...
Installer.prototype.loadShrinkwrap = function (cb) {
  ...
  readShrinkwrap.andInflate(this.idealTree, iferr(cb, function () {
    computeMetadata(_this6.idealTree);
    cb();
  }));
};
...
Installer.prototype.printInstalled = function (cb) {
  ...
  var diffs = this.differences;
  if (!this.idealTree.error && this.idealTree.removedChildren) {
    var deps = this.currentTree.package.dependencies || {};
    var dev = this.currentTree.package.devDependencies || {};
    this.idealTree.removedChildren.forEach(function (r) {
      if (diffs.some(function (d) {
        return d[0] === 'remove' && d[1].path === r.path;
      })) return;
  ...
  return Bluebird.try(function () {
    if (!_this7.auditSubmission) return;
    return Bluebird.resolve(_this7.auditSubmission).timeout(10000).catch(function () {
      return null;
    });
  }).then(function (auditResult) {
    // maybe write audit report w/ hash of pjson & shrinkwrap for later reading by `npm audit`
    if (npm.config.get('json')) {
      return _this7.printInstalledForJSON(diffs, auditResult);
    } else if (npm.config.get('parseable')) {
      return _this7.printInstalledForParseable(diffs, auditResult);
    } else {
      return _this7.printInstalledForHuman(diffs, auditResult);
    }
  }).asCallback(cb);
};
...
Installer.prototype.printInstalledForHuman = function (diffs, auditResult) {
  ...
  if (this.args.length && (added || updated)) {
    report += this.args.map(function (p) {
...
Installer.prototype.printInstalledForParseable = function (diffs) {
  ...
  diffs.forEach(function (action) {
    ...
    output(mutation + '\t' + moduleName(child) + '\t' + (child.package ? child.package.version : '') + '\t' + (child.path ? path.relative(self.where, child.path) : '') + '\t' + (previousVersion || '') + '\t' + (previousPath || ''));
  });
};
...
Installer.prototype.archyDebugLogicalTree = function (tree) {
  ...
  function expandTree(tree) {
    ...
    return {
      label: packageId(tree),
      nodes: tree.requires.filter(function (tree) {
        return !seen.has(tree) && !tree.removed;
      }).sort(byName).map(expandTree)
    };
  }
