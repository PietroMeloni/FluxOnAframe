...
var addBundled = void 0;
var childPath = require('../utils/child-path.js');
var createChild = require('./node.js').create;
var fetchPackageMetadata = void 0;
...
module.exports = function (tree, sw, opts, finishInflating) {
  ...
  return inflateShrinkwrap(tree.path, tree, sw.dependencies, opts).then(function () {
    return finishInflating();
  }, finishInflating);
};
...
function inflateShrinkwrap(topPath, tree, swdeps, opts) {
  ...
  return BB.each(Object.keys(swdeps), function (name) {
    var sw = swdeps[name];
    var dependencies = sw.dependencies || {};
    var requested = realizeShrinkwrapSpecifier(name, sw, topPath);
    return inflatableChild(onDisk[name], name, topPath, tree, sw, requested, opts).then(function (child) {
...
function tarballToVersion(name, tb) {
  var registry = quotemeta(npm.config.get('registry')).replace(/https?:/, 'https?:').replace(/([^/])$/, '$1/');
  var matchRegTarball = void 0;
  if (name) {
    var nameMatch = quotemeta(name);
    matchRegTarball = new RegExp('^' + registry + nameMatch + '/-/' + nameMatch + '-(.*)[.]tgz$');
  } else {
    matchRegTarball = new RegExp('^' + registry + '(.*)?/-/\\1-(.*)[.]tgz$');
  }
  var match = tb.match(matchRegTarball);
  if (!match) return;
...
function inflatableChild(onDiskChild, name, topPath, tree, sw, requested, opts) {
  ...
  var usesIntegrity = requested.registry || requested.type === 'remote' || requested.type === 'file';
  var regTarball = tarballToVersion(name, sw.version);
  if (regTarball) {
    sw.resolved = sw.version;
    sw.version = regTarball;
  }
  if (sw.requires) Object.keys(sw.requires).map(function (_) {
    sw.requires[_] = tarballToVersion(_, sw.requires[_]) || sw.requires[_];
  });
  var modernLink = requested.type === 'directory' && !sw.from;
...
function isGit(sw) {
  var version = npa.resolve(sw.name, sw.version);
...
function makeFakeChild(name, topPath, tree, sw, requested) {
  ...
  if (!sw.bundled) {
    var bundleDependencies = Object.keys(sw.dependencies || {}).filter(function (d) {
...
function fetchChild(topPath, tree, sw, requested) {
  return fetchPackageMetadata(requested, topPath).then(function (pkg) {
    pkg._from = sw.from || requested.raw;
    pkg._optional = sw.optional;
    pkg._development = sw.dev;
    pkg._inBundle = false;
    return addBundled(pkg).then(function () {
      return pkg;
    });
  }).then(function (pkg) {
...
//# sourceMappingURL=inflate-shrinkwrap.js.map