...
var PKGLOCK_VERSION = require('../npm.js').lockfileVersion;
...
function readShrinkwrap(child, next) {
  ...
  child.isTop && maybeReadFile('package-lock.json', child), child.isTop && maybeReadFile('package.json', child), function (shrinkwrap, lockfile, pkgJson) {
    ...
    var name = shrinkwrap ? 'npm-shrinkwrap.json' : 'package-lock.json';
    var parsed = parsePkgLock(shrinkwrap || lockfile, name);
...
function maybeReadFile(name, child) {
  return readFileAsync(path.join(child.path, name), 'utf8').catch({ code: 'ENOENT' }, function () {
    return null;
  });
}
...
function parsePkgLock(str, filename) {
  ...
  try {
    ...
    if (isDiff(str)) {
      ...
      try {
        var ours = parseJSON(pieces.ours);
        var theirs = parseJSON(pieces.theirs);
...
//# sourceMappingURL=read-shrinkwrap.js.map