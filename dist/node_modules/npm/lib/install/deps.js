...
function doesChildVersionMatch(child, requested, requestor) {
  ...
  if (requested.type === 'git' && child.fromShrinkwrap) {
    var fromSw = child.package._from ? npa(child.package._from) : child.fromShrinkwrap;
...
function computeMetadata(tree, seen) {
  ...
  if (tree.isTop) {
    ...
    try {
      for (var _iterator2 = Object.keys(devDeps)[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var _name = _step2.value;

        if (findChild(_name, devDeps[_name])) continue;
        tree.missingDevDeps[_name] = devDeps[_name];
      }
    } catch (err) {
  ...
  tree.children.filter(function (child) {
    ...
    return computeMetadata(child, seen);
  });
...
function packageRelativePath(tree) {
  ...
  return isLocal ? requested.fetchSpec : (tree.isLink || tree.isInLink) && !preserveSymlinks() ? tree.realpath : tree.path;
}
...
exports.getAllMetadata = function (args, tree, where, next) {
  asyncMap(args, function (arg, done) {
    ...
    if (spec.type !== 'file' && spec.type !== 'directory' && (spec.name == null || spec.rawSpec === '')) {
      return fs.stat(path.join(arg, 'package.json'), function (err) {
        if (err) {
...
exports.loadRequestedDeps = function (args, tree, saveToDependencies, log, next) {
  ...
  asyncMap(args, function (pkg, done) {
    ...
    resolveWithNewModule(pkg, tree, log.newGroup('loadRequestedDeps'), iferr(depLoaded, function (child, tracker) {
      ...
      var types = ['dependencies', 'devDependencies', 'optionalDependencies'];
      if (child.save) {
...
exports.removeDeps = function (args, tree, saveToDependencies, next) {
  ...
  try {
    for (var _iterator4 = args[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
      ...
      try {
        for (var _iterator5 = pkgToRemove.requiredBy[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
          var parent = _step5.value;

          parent.requires = parent.requires.filter(function (child) {
            return child !== pkgToRemove;
          });
        }
      } catch (err) {
      ...
      pkgToRemove.requiredBy = pkgToRemove.requiredBy.filter(function (parent) {
        return parent !== tree;
      });
...
function flagAsRemoving(toRemove, seen) {
  ...
  toRemove.requires.forEach(function (required) {
...
function isDepOptional(tree, name, pkg) {
  ...
  var prodDeps = tree.package.dependencies;
...
function prefetchDeps(tree, deps, log, next) {
  ...
  var seen = new Set();
  var finished = andFinishTracker(log, next);
  var fpm = BB.promisify(fetchPackageMetadata);
  resolveBranchDeps(tree.package, deps).then(function () {
    return finished();
  }, finished);

  function resolveBranchDeps(pkg, deps) {
    return BB.resolve(null).then(function () {
      var allDependencies = Object.keys(deps).map(function (dep) {
      ...
      if (skipOptional) {
        ...
        allDependencies = allDependencies.filter(function (dep) {
          return !optDeps[dep.name];
        });
      }
      return BB.map(allDependencies, function (dep) {
        seen.add(dep.toString());
        return fpm(dep, '', { tracker: log.newItem('fetchMetadata') }).then(function (pkg) {
...
function reportBundleOverride(child, log) {
  var code = 'EBUNDLEOVERRIDE';
  var top = getTop(child.fromBundle);
  var bundlerId = packageId(child.fromBundle);
  if (!top.warnings.some(function (w) {
    return w.code === code;
  })) {
    var err = new Error(bundlerId + ' had bundled packages that do not match the required version(s). They have been replaced with non-bundled versions.');
    err.code = code;
...
function resolveWithNewModule(pkg, tree, log, next) {
  ...
  return isInstallable(pkg, function (err) {
    var installable = !err;
    addBundled(pkg, function (bundleErr) {
      ...
      if (pkg._shrinkwrap && pkg._shrinkwrap.dependencies) {
        return inflateShrinkwrap(child, pkg._shrinkwrap, function (swErr) {
...
function preserveSymlinks() {
  ...
  var value = process.env.NODE_PRESERVE_SYMLINKS;
...
var earliestInstallable = exports.earliestInstallable = function (requiredBy, tree, pkg, log) {
  ...
  var undeletedMatches = tree.children.filter(undeletedModuleMatches);
  if (undeletedMatches.length) {
    ...
    if (tree === requiredBy) {
      undeletedMatches.forEach(function (pkg) {
