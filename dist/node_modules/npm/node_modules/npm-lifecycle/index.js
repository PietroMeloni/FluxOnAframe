...
var DEFAULT_NODE_GYP_PATH = resolveFrom(__dirname, 'node-gyp/bin/node-gyp');
...
function hookStat(dir, stage, cb) {
  var hook = path.join(dir, '.hooks', stage);
  var cachedStatError = hookStatCache.get(hook);
  ...
  return new Promise(function (resolve, reject) {
    ...
    hookStat(opts.dir, stage, function (statError) {
      ...
      validWd(wd || path.resolve(opts.dir, pkg.name), function (er, wd) {
        ...
        lifecycle_(pkg, stage, wd, opts, env, function (er) {
          if (er) return reject(er);
          return resolve();
        });
      });
...
function shouldPrependCurrentNodeDirToPATH(opts) {
  var cfgsetting = opts.scriptsPrependNodePath;
    ...
    if (i.charAt(0) !== '_') {
      ...
      if (data[i] && _typeof(data[i]) === 'object') {
        try {
          ...
          var d = data[i];
          makeEnv({ name: d.name, version: d.version, path: d.path }, opts, envKey + '_', env);
        }
      } else {
