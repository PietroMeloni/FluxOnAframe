...
function unpublish(uri, params, cb) {
  ...
  assert(params && (typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object', 'must pass params to unpublish');
  assert(typeof cb === 'function', 'must pass callback to unpublish');

  var ver = params.version;
  var auth = params.auth;
  assert(auth && (typeof auth === 'undefined' ? 'undefined' : _typeof(auth)) === 'object', 'must pass auth to unpublish');
  ...
  this.get(uri + '?write=true', options, function (er, data) {
    ...
    if (latestVer === ver) {
      data['dist-tags'].latest = Object.getOwnPropertyNames(versions).sort(semver.compareLoose).pop();
    }
