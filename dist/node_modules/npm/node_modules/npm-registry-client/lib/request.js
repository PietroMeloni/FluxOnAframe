...
function regRequest(uri, params, cb_) {
  ...
  assert(params && (typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object', 'must pass params to request');
...
function makeRequest(uri, params, cb_) {
  ...
  var opts = this.initialize(parsed, params.method, useCorgi ? 'application/vnd.npm.install-v1+json; q=1.0, application/json; q=0.8, */*' : 'application/json', headers);
...
function requestDone(method, where, cb) {
  return function (er, response, data) {
    ...
    if (parsed && parsed.error && _typeof(parsed.error) !== 'object' || response.statusCode >= 400) {
