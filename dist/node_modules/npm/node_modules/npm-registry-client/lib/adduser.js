...
function adduser(uri, params, cb) {
  ...
  assert(params && (typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object', 'must pass params to adduser');
  assert(typeof cb === 'function', 'must pass callback to adduser');

  assert(params.auth && _typeof(params.auth), 'must pass auth to adduser');
  ...
  this.request(uri, Object.assign({}, options), function (error, data, json, response) {
    if (!error || !response || response.statusCode !== 409) {
      return cb(error, data, json, response);
    }

    client.log.verbose('adduser', 'update existing user');
    return client.request(uri + '?write=true', { auth: auth }, function (er, data, json, response) {
      if (er || data.error) {
  ...
  function done(token, cb) {
    return function (error, data, json, response) {
      ...
      if (!error) {
        error = new Error((response && response.statusCode || '') + ' ' + 'Could not create user\n' + JSON.stringify(data));
      }
