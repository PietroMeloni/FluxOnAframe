...
subcommands.ls = function (uri, params, cb) {
  ...
  if (params.team) {
    var reqUri = apiUri(uri, 'team', params.scope, params.team, 'user') + uriParams;
...
function apiUri(registryUri) {
  var path = Array.prototype.slice.call(arguments, 1).map(encodeURIComponent).join('/');
...
function teamAssertions(subcommand, uri, params, cb) {
  ...
  assert(subcommands.hasOwnProperty(subcommand), 'team subcommand must be one of ' + Object.keys(subcommands));
  assert(typeof uri === 'string', 'registry URI is required');
  assert((typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object', 'params are required');
  assert(_typeof(params.auth) === 'object', 'auth is required');
  ...
  if (subcommand === 'edit') {
    assert(_typeof(params.users) === 'object' && params.users.length != null, 'users is required');
  }
}
