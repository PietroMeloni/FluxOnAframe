...
var linkIfExists = BB.promisify(require('gentle-fs').linkIfExists);
var cmdShimIfExists = BB.promisify(require('cmd-shim').ifExists);
var open = BB.promisify(fs.open);
var close = BB.promisify(fs.close);
var read = BB.promisify(fs.read, { multiArgs: true });
var chmod = BB.promisify(fs.chmod);
var readFile = BB.promisify(fs.readFile);
var writeFileAtomic = BB.promisify(require('write-file-atomic'));
...
function isHashbangFile(file) {
  return open(file, 'r').then(function (fileHandle) {
    return read(fileHandle, Buffer.alloc(2), 0, 2, 0).spread(function (_, buf) {
      if (!hasHashbang(buf)) return [];
      return read(fileHandle, Buffer.alloc(2048), 0, 2048, 0);
    }).spread(function (_, buf) {
      return buf && hasCR(buf);
    }, function () {
      return false;
    }).finally(function () {
...
function linkMans(pkg, folder, parent, gtop, opts) {
  ...
  return BB.map(manpages, function (man) {
