...
function checkEngine(target, npmVer, nodeVer, force, strict, cb) {
  ...
  if (nodev && eng.node && !semver.satisfies(nodev, eng.node) || eng.npm && !semver.satisfies(npmVer, eng.npm)) {
    var er = new Error(util.format('Unsupported engine for %s: wanted: %j (current: %j)', target._id, eng, { node: nodev, npm: npmVer }));
    er.code = 'ENOTSUP';
    er.required = eng;
    er.pkgid = target._id;
    if (strict) {
...
function checkPlatform(target, force, cb) {
  ...
  if (!osOk || !cpuOk) {
    var er = new Error(util.format('Unsupported platform for %s: wanted %j (current: %j)', target._id, target, { os: platform, cpu: arch }));
