...
var defaultRules = ['.npmignore', '.gitignore', '**/.git/', '**/.svn/', '**/.hg/', '**/CVS/', '/.lock-wscript', '/.wafpickle-*', '/build/config.gypi', 'npm-debug.log', '**/.npmrc', '.*.swp', '.DS_Store', '._*', '*.orig', 'package-lock.json'];
  ...
  return function (_Class) {
    ...
    _createClass(Walker, [{
      ...
      value: function onPackageJson(ig, pkg, then) {
        ...
        var rules = [pkg.browser ? '!' + pkg.browser : '', pkg.main ? '!' + pkg.main : '', '!@(readme|license|licence|notice|changes|changelog|history){,.*}'].filter(function (f) {
          return f;
        }).join('\n') + '\n';
