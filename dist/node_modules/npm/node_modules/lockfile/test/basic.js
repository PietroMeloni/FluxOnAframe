...
test('wait and stale together', function (t) {
  ...
  lockFile.lock('stale-wait-lock', function (er) {
    ...
    lockFile.lock('stale-wait-lock', opt, function (er) {
      if (!er) t.fail('got second lock?  that unpossible!');else t.pass('second lock failed, as i have foreseen it');
...
test('stale windows file tunneling test', function (t) {
  ...
  lockFile.lock('stale-windows-lock', opts, function (er) {
    if (!er) t.fail('got second lock?  impossible, windows file tunneling problem!');else t.pass('second lock failed, windows file tunneling problem fixed');
    t.end();
  });
});
