...
function fromHex(hexDigest, algorithm, opts) {
  var optString = opts && opts.options && opts.options.length ? '?' + opts.options.join('?') : '';
...
function fromData(data, opts) {
  ...
  var optString = opts.options && opts.options.length ? '?' + opts.options.join('?') : '';
  return algorithms.reduce(function (acc, algo) {
    var digest = crypto.createHash(algo).update(data).digest('base64');
    var hash = new Hash(algo + '-' + digest + optString, opts);
    if (hash.algorithm && hash.digest) {
...
function checkData(data, sri, opts) {
  ...
  var newSri = parse({ algorithm: algorithm, digest: digest });
