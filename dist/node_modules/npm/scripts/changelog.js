...
var branch = process.argv[2] || 'origin/latest';
var log = execSync('git log --reverse --pretty=\'format:%h %H%d %s (%aN)%n%b%n---%n\' ' + branch + '...').toString().split(/\n/);

main();

function shortname(url) {
  var matched = url.match(/https:\/\/github.com\/([^/]+\/[^/]+)\/(?:pull|issues)\/(\d+)/);
...
function printCommit(c) {
  ...
  if (c.fixes) {
    ...
    var _label = shortname(c.prurl);
    if (_label) {
      console.log('  [' + _label + '](' + c.prurl + ')');
    } else {
      console.log('  [#](' + c.prurl + ')');
    }
  }
  var msg = c.message.replace(/^\s+/mg, '').replace(/^[-a-z]+: /, '').replace(/^/mg, '  ').replace(/\n$/, '')
  // backtickify package@version
  .replace(/^(\s*[^@\s]+@\d+[.]\d+[.]\d+)(\s*\S)/g, '$1:$2').replace(/\b([^@\s]+@\d+[.]\d+[.]\d+)\b/g, '`$1`')
  // linkify commitids
  .replace(/\b([a-f0-9]{7,8})\b/g, '[`$1`](https://github.com/npm/npm/commit/$1)').replace(/\b#(\d+)\b/g, '[#$1](https://github.com/npm/npm/issues/$1)');
  console.log(msg);
  if (c.credit) {
